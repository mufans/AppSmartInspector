...
def test_sched_latency_hint_over_budget():
    ...
    data = {
        ...
        "summary": {"total_threads": 2, "worst_thread": "main"},
    }
...
# --- Dimension hints integration ---

from smartinspector.agents.deterministic import _compute_dimension_hints


def test_compute_dimension_hints_with_data():
    data = {
        "dimensions": {
            "lock_contention": {
                "threads": [
    ...
    hints = _compute_dimension_hints(data, 16.67)
    assert "[锁竞争]" in hints


def test_compute_dimension_hints_empty():
    hints = _compute_dimension_hints({}, 16.67)
    assert hints == ""


def test_compute_dimension_hints_no_dimensions():
    hints = _compute_dimension_hints({"cpu_usage": {}}, 16.67)
    assert hints == ""
